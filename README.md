...
[下载批注修订版 PPT v8：77 页](docs/presentations/MYA_Complete_v8_20260916.pptx)：明确手机发起配对的目标设计、当前Demo差别，以及钱包使用已有授权或重新申请授权的条件。
...
支持用户名和密码登录、签名确认新设备、统一 Agent 元数据目录，以及只读／审批／设备管理权限。**同一账号不等于同一审批设备**：新设备不会继承原手机私钥，也不能批准原手机的关系。请先在已有配对的手机浏览器注册，再由该手机确认其他设备。[操作说明](docs/ACCOUNT_AND_DEVICES.md) · [撤销场景](docs/REVOCATION_SCENARIOS.md)。
